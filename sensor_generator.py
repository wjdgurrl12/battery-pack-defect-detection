...
def build_message(row: dict, seq: int, label: str | None = None) -> dict:
    ...
    label 은 명세(5.6)의 자리라 남겨 뒀지만 지금은 늘 None 이다. 발행 직전
    주입(3단계)이 있던 시절 "defect" 를 실어 보내던 자리다 - 데모 팩은 고장이
    데이터에 들어 있고 정답은 database.DEMO_PACKS 가 들고 있어 라벨이 필요
    없다. 필드를 없애면 컨슈머와 명세가 같이 움직여야 하므로 자리만 남긴다.
    """
    return {
        # --- DB 에 없어서 여기서 만드는 값 ---
        ...
        # 정답 라벨 자리(명세 5.6). 발행 직전 주입이 없어진 지금은 늘 None.
        "label": label,
    }


# ---------------------------------------------------------------------------
# 3단계: 이상치 주입 - 2026-08-30 에 걷어냈다
# ---------------------------------------------------------------------------
# 원본 50팩이 전부 정상이던 시절에는 검출을 시험하려면 발행 직전에 값을
# 흔드는 수밖에 없었다(AnomalyPlan · make_anomalous · --anomaly-every).
# 데모 팩이 생기면서 고장이 데이터 자체에 들어갔고(database.DEMO_SERIALS),
# 발행 직전에 또 흔들면 오히려 정답표와 대조가 안 되므로 단계 전체를 지웠다.
# 같은 시절의 일회성 주입 도구 inject_anomalies.py 는 old/ 로 옮겼다 -
# 지워진 코드는 git 이력에 있다. 단계 번호는 문서·주석이 참조하고 있어
# 4단계부터를 당겨 매기지 않는다.
...
def main() -> None:
    ...
    started = time.time()
    sent = run(args.serial, args.mode, args.interval, args.limit, demo)
